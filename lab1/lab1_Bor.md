...
2. **Метод запроса** : В нашем случае это метод GET, для загрузки страницы испольузется именно этот метод, так как именно он, позволяет получить содержимое ресурса, к которму мы обращаемся
